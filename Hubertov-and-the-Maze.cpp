              ...
              struct cell pi[][MAXW + 1])
{
    ...
    for(idRow = 1; idRow <= H; idRow++)
    {
        for(idColumn = 1; idColumn <= W; idColumn++)
        {
               ...
               (color[U.coord_y][U.coord_x] == WHITE))
            {
                ...
                //Elegir el arbol con mayor profundidad
                depth = ((f[U.coord_x][U.coord_y] - d[U.coord_x][U.coord_y]) / 2) + 1;
                if(depth > maxDepth)
                {
                    maxDepth = depth;
                    root1 = U;
    ...
    diameter1 = d[root1.coord_x][root1.coord_y];
    printf("\n %d \n", diameter1);
    printf("------");
    printf("%d %d", root1.coord_x, root1.coord_y);

    if(root2.coord_x != -1 && root2.coord_y != -1)
    {
        //BFS_Maze(Maze, W, H, root1, color, d)
    }
    else
    {
